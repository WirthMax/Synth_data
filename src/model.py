import argparse, math, re, os, time, json
import numpy as np
...
def to_input(adu):
    ...
    return (np.log1p(np.clip(x, 0, ADU_MAX)) / np.float32(np.log1p(ADU_MAX))).astype(np.float32)
...
class MMAPView(torch.utils.data.Dataset):
    ...
    def __getitem__(self, k):
        ...
        return torch.from_numpy(to_input(a)).permute(2, 0, 1), torch.from_numpy(self.y[i])
...
def load(path, val_frac=0.2, seed=0):
    ...
    return (MMAPView(images, theta, tr, augment=True),
            MMAPView(images, theta, va, augment=False),
            paths, lo, hi, images.shape[-1])
...
def evaluate(model, loader, device):
    ...
    for x, y in loader:
        ...
        MU.append(mu.cpu().numpy()); LV.append(lv.cpu().numpy()); Y.append(y.numpy())
    mu = np.concatenate(MU); lv = np.concatenate(LV); y = np.concatenate(Y)
    ...
    return dict(r2=r2, rmse=np.sqrt(ss_res), sigma=sig01,
                z_mean=z.mean(0), z_std=z.std(0), mu=mu, y=y, lv=lv)
...
def train(args):
    ...
    print(f"device {device} | train {len(tr)} val {len(va)} | {len(paths)} parameters "
          f"| {in_ch} channels")
    dl_tr = torch.utils.data.DataLoader(tr, batch_size=args.batch, shuffle=True, drop_last=False)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for x, y in dl_tr:
            ...
            loss = F.mse_loss(mu, model.to_logit(y)) if ep <= args.warmup else model.nll(mu, lv, y)
            if not torch.isfinite(loss):
                print(f"non-finite loss, ep {ep}: y_ok={torch.isfinite(y).all().item()} "
        ...
        if ep % args.every == 0 or ep == args.epochs:
            ...
            print(f"ep {ep:4d}  loss {tot / max(k, 1):8.4f}  val median R2 {med:6.3f}  "
                  f"mean RMSE {ev['rmse'].mean():.4f}  z-std {zs:5.2f}  "
                  f"{time.time() - t0:5.0f}s", flush=True)
            score = float(ev["rmse"].mean())
            if score < best:
                best = score
                torch.save(dict(model=model.state_dict(), paths=paths, lo=lo, hi=hi,
                                in_ch=in_ch, width=args.width, depth=args.depth),
    ...
    for j, name in enumerate(paths):
        n = str(name)
        print(f"{ev['r2'][j]:7.3f} {ev['rmse'][j]:9.4f} {ev['sigma'][j]:9.4f} "
              f"{ev['rmse'][j] * span[j]:11.4g} {ev['z_std'][j]:6.2f}  {n}")
        report[n] = dict(r2=float(ev["r2"][j]), rmse01=float(ev["rmse"][j]),
                         rmse_raw=float(ev["rmse"][j] * span[j]),
                         pred_sd01=float(ev["sigma"][j]),
                         z_mean=float(ev["z_mean"][j]), z_std=float(ev["z_std"][j]))
...
def main():
    ...
    t.add_argument("--warmup", type=int, default=20, help="epochs of MSE before switching to NLL")
